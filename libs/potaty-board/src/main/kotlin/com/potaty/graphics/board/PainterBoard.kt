...
import com.potaty.graphics.board.CrossingResources.isConnectable
import com.potaty.graphics.board.PotatyBoard.CrossPoint
...
internal class PainterBoard(internal val bound: Rect) {
     ...
     * A list of [CrossPoint] will be returned to let [PotatyBoard] able to adjust and draw the
     * adjusted character of the connection points.
     *
     * The main reason why it is required to let [PotatyBoard] draws the connection points is the
     ...
     * position won't be overwritten.
     */
    fun fill(position: Point, bitmap: PotatyBitmap, highlight: Highlight): List<CrossPoint> {
        if (bitmap.isEmpty()) {
            return emptyList()
        }
